...
./configure "--prefix=${basedir}/build/${mode}/${processor}/installdir" --without-gnu-ld --disable-doxygen --disable-dot --disable-latex-docs --disable-static --host=x86_64-pc-mingw32 --target=x86_64-pc-mingw32
make && make install
errorcode=$?

mkdir -p ${basedir}/native/${mode}/lib ${basedir}/native/${mode}/include ${basedir}/native/${mode}/config

res=$(cp ${basedir}/build/${mode}/${processor}/installdir/bin/*.dll ${basedir}/native/${mode}/lib/ 2>&1)
res=$(cp ${basedir}/build/${mode}/${processor}/installdir/lib/*.dll ${basedir}/native/${mode}/lib/ 2>&1)
res=$(cp ${basedir}/build/${mode}/${processor}/installdir/lib/*.a ${basedir}/native/${mode}/lib/ 2>&1)
res=$(cp ${basedir}/build/${mode}/${processor}/installdir/lib/*.lib ${basedir}/native/${mode}/lib/ 2>&1)
