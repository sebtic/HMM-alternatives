...
./configure "--prefix=/work/target/build/${mode}/${processor}/installdir" --without-gnu-ld --disable-doxygen --disable-dot --disable-latex-docs --disable-static --host=i686-pc-mingw32 --build=x86_64-pc-linux
make && make install
errorcode=$?

mkdir -p /work/target/native/${mode}/lib /work/target/native/${mode}/include /work/target/native/${mode}/config

res=$(cp /work/target/build/${mode}/${processor}/installdir/bin/*.dll /work/target/native/${mode}/lib/ 2>&1)
res=$(cp /work/target/build/${mode}/${processor}/installdir/lib/*.dll /work/target/native/${mode}/lib/ 2>&1)
res=$(cp /work/target/build/${mode}/${processor}/installdir/lib/*.a /work/target/native/${mode}/lib/ 2>&1)
res=$(cp /work/target/build/${mode}/${processor}/installdir/lib/*.lib /work/target/native/${mode}/lib/ 2>&1)
