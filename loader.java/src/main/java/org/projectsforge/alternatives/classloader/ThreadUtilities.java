 ...
 * along with this file. If not, see <a href="http://www.gnu.org/licenses">GNU
 * Licenses</a>.
 */
...
public final class ThreadUtilities {
  ...
  public static Thread[] getAllDaemonThreads() {
    final Thread[] allThreads = ThreadUtilities.getAllThreads();
  ...
  /**
   * Get a list of all thread groups. Since there is always at least one thread
   * group (the root), this method never returns a null or empty array.
   * 
   * @return an array of thread groups
   */
  public static ThreadGroup[] getAllThreadGroups() {
    final ThreadGroup root = ThreadUtilities.getRootThreadGroup();
  ...
  // Threads
  /**
   * Get a list of all threads. Since there is always at least one thread, this
   * method never returns null or an empty array.
   * 
  ...
  /**
   * Get the thread who's lock is blocking the given thread. A null is returned
  ...
  public static Thread getBlockingThread(final Thread blockedThread) {
    final ThreadInfo info = ThreadUtilities.getThreadInfo(blockedThread);
    if (info == null) {
      return null;
    }
  ...
  public static Thread[] getGroupThreads(final String name) {
    final ThreadGroup group = ThreadUtilities.getThreadGroup(name);
    if (group == null) {
      return null;
    }
  ...
  /**
   * Get a list of all threads in a thread group. An empty array is returned if
   * there are no threads in the group.
   * 
   * @param group
  ...
  /**
   * Get the root thread group in the thread group tree. Since there is always a
   * root thread group, this method never returns null.
   * 
   * @return the root thread group
   */
  public static ThreadGroup getRootThreadGroup() {
    if (ThreadUtilities.rootThreadGroup != null) {
      return ThreadUtilities.rootThreadGroup;
    }
    ...
    ThreadUtilities.rootThreadGroup = tg;
    return tg;
  }
  ...
  public static Thread getThread(final long id) {
    final Thread[] threads = ThreadUtilities.getAllThreads();
  ...
  /**
   * Get the thread with the given name. A null is returned if no such thread is
   * found. If more than one thread has the same name, the first one found is
   * returned.
   * 
   * @param name
  ...
  /**
   * Get the thread for the given thread info. A null is returned if the thread
   * cannot be found.
   * 
   * @param info
  ...
  public static ThreadGroup getThreadGroup(final String name) {
    ...
    final ThreadGroup[] groups = ThreadUtilities.getAllThreadGroups();
  ...
  /**
   * Get the thread info for the thread with the given ID. A null is returned if
   * no such thread info is found.
   * 
   * @param id
   *          the thread ID to search for
  ...
  /**
   * Get the thread info for the thread with the given name. A null is returned
   * if no such thread info is found. If more than one thread has the same name,
   * the thread info for the first one found is returned.
   * 
   * @param name
   *          the thread name to search for
   * @return the thread info, or null if not found
  ...
  /**
   * Get the thread info for the given thread. A null is returned if the thread
